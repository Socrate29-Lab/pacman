...
void move_Pacman(int last_key,int *posY, int *posX, int *score, int *life, int *statut,
    int *timer_super, int *ghost1, int *ghost2, int *ghost3, int *timerG1, int *timerG2, int *timerG3){
    ...
    if(next_move >= 6 && next_move <= 8){
        if(*statut == 0){
            *life = 0;
            ...
            if(next_move == 6){
                *ghost1 = 0;
                *timerG1 = SDL_GetTicks();
            }
            if(next_move == 7){
                *ghost2 = 0;
                *timerG2 = SDL_GetTicks();
            }
            if(next_move == 8){
                *ghost3 = 0;
                *timerG3 = SDL_GetTicks();
            }
            *score += 200;
        }    
    }
